...
- Insurance/reinsurance sourcing (item 9 above) used Aon + Swiss Re only — Gallagher Re and Munich Re haven't been pulled from yet; noted as sufficient for that pass, not exhaustive, in `refresh-log.json`'s row for this data type.
...
- JO's underlying index (Bloomberg Coffee Subindex) may track Arabica rather than Robusta futures, which would be in tension with the entry's existing Vietnam/Indonesia/Robusta-coded thesis language — surfaced during the ENSO backtest pass (item 10 above), a WebSearch was inconclusive, not yet resolved. Moot for trading (JO is delisted) but worth fixing for research-integrity reasons if anyone revisits this entry.
